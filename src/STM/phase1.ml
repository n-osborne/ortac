...
let rec unsafe_term ~old_state_name ~state_arg ~driver (t : Tterm.term) :
  ...
  match t.t_node with
  | Told t0 -> (
      match is_state t0.t_node with
      | None ->
          unsupported "old operator used on a term that is not the state (STM)"
  ...
  | Tquant (Tterm.Tlambda, args, t) ->
      ...
      in
      efun args t
  | Tquant (Tterm.(Tforall | Texists), _, _) ->
      unsupported "ill formed quantification"
  | Tbinop (op, t1, t2) -> (
      match op with
      | Tterm.Tand ->
...
let term ~old_state_name ~state_arg ~driver (_fail : string) t =
  try Ok (unsafe_term ~old_state_name ~state_arg ~driver t)
  ...
  let has_dup l =
    ...
    List.fold_right
      (fun ele (dup, prev) ->
        ( (match prev with
          | None -> dup
          | Some prev -> if String.equal ele prev then Some ele else dup),
          Some ele ))
      sorted (None, None)
    |> fst
  in
  let check_dups =
    List.map (fun (l, _) -> l.Gospel.Symbols.ls_name.id_str) fields |> has_dup
  in
  (match check_dups with
  | None -> ()
  | Some dup -> raise (Failure ("duplicate model: " ^ dup)));
  let models =
    List.map
      (fun (l, _) ->
        ( l.Gospel.Symbols.ls_name.id_str,
          Option.get l.Gospel.Symbols.ls_value |> Translate.type_of_ty ~driver
        ))
      fields
  in
  { type_ with models }
...
let type_ ~(driver : Drv.t) ~ghost (td : Tast.type_declaration) : Drv.t =
  ...
  let process ~(type_ : Ts.type_) (spec : Tast.type_spec) =
    let mutable_ =
      Mutability.(max type_.Ts.mutable_ (type_spec ~driver spec))
    in
...
let with_checks ~old_state_name ~state_arg ~driver (checks : Tterm.term list)
    (value : Translated.value) : Translated.value =
  let txt = "silly" in
  let checks =
    List.map
      (fun t ->
...
let with_pre ~old_state_name ~state_arg ~driver ~term_printer pres
  ...
  let preconditions =
    List.map
      (fun t ->
        ...
        ({ txt; loc; translation } : Translated.term))
      pres
  in
  { value with preconditions }

let with_post ~old_state_name ~state_arg ~driver ~term_printer
    (posts : Tterm.term list) (rets : Tast.lb_arg list)
    (value : Translated.value) =
  let contains_rets (t : Tterm.term) : bool =
    ...
    List.fold_right
      (fun ret acc ->
        try
          let vs = Tast_helper.vs_of_lb_arg ret in
  ...
  let marks = List.map (fun t -> contains_rets t) posts in
  (*true if contains ret, false if not*)
  let postconditions =
    let open Translated in
    List.map2
      (fun post contains_returns -> { post; contains_returns })
      postconditions marks
  in
  { value with postconditions }

let assert_false_case = case ~guard:None ~lhs:[%pat? _] ~rhs:[%expr false]

(*each exception has multiple patterns, terms*)
(*the second element of xposts is passed in as the ptlist in xpost fn below*)
let with_xposts ~old_state_name ~state_arg ~driver
    (xposts : (Ttypes.xsymbol * (Tterm.pattern * Tterm.term) list) list)
    (value : Translated.value) =
  (*xpost processes one raises into a case list*)
  let xpost
      ((exn : Ttypes.xsymbol), (ptlist : (Tterm.pattern * Tterm.term) list)) =
    let name : string = exn.Ttypes.xs_ident.id_str in
    let cases =
      List.map
        (fun (p, t) ->
          term ~old_state_name ~state_arg ~driver "xpost" t
          |> Result.map (fun (t : expression) ->
                 (*turn the term into a case*)
                 case ~guard:None
                   ~lhs:(Translation.xpost_pattern ~driver name p.Tterm.p_node)
                     (*make an xpost pattern*)
                   ~rhs:t))
        (* XXX ptlist must be rev because the cases are given in the
           reverse order by gospel <- false*)
        (List.rev ptlist)
      (*<- earlier phase ensures that this is
  ...
  in
  let xpostconditions : Translated.xpost list =
    (*turn each tast xpost into a translated xpost*)
    let open Translated in
    List.map
      (fun xp ->
        let xs = fst xp in
...
let value ~old_state_name ~state_arg ~driver ~ghost (vd : Tast.val_description)
    : Drv.t =
  ...
  *)
  ...
  let process ~value (spec : Tast.val_spec) =
    ...
    let value =
      ...
      |> with_post ~old_state_name ~state_arg ~driver ~term_printer spec.sp_post
           spec.sp_ret
  ...
  in
  let driver = Drv.add_translation value_item driver in
  driver

(*starts with empty driver (from ortac_core.signature)*)
let signature ~driver s : Drv.t * string =
  let state_arg (args : Tast.lb_arg list) =
    List.find_opt
      (fun arg ->
  ...
  ( List.fold_left
      (fun driver (sig_item : Tast.signature_item) ->
        ...
        | Sig_val (vd, ghost) when vd.vd_args <> [] ->
            let state_arg =
              try Option.map Tast_helper.vs_of_lb_arg (state_arg vd.vd_args)
