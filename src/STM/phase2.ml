...
let safe_add (key : string) v (m : 'a S.t) =
  match S.find_opt key m with
...
let rec typ_of_type_ (error : string) (ty : type_) =
  ...
  | [ arg1; arg2 ] -> (
      match ty.name with
      | "tuple2" -> Tuple (typ_of_type_ error arg1, typ_of_type_ error arg2)
      | _ -> unsupported_type ty.name)
  | _ -> unsupported_type ty.name
...
let rec mk_qcheck (typ : Ast3.typ) : expression =
  ...
  | Integer ->
      raise (Failure "cannot make generator for Gospel stdlib type Integer")
  | Char -> [%expr frequency [ (5, printable); (15, char) ]]
  | String ->
      [%expr
        frequency [ (5, small_string ~gen:printable); (20, string ~gen:char) ]]
  | Bool -> [%expr bool]
  | Unit -> [%expr unit]
...
let mk_ocaml_var (v : Translated.ocaml_var) : Ast3.ocaml_var =
  { name = v.name; label = v.label; typ = typ_of_type_ v.name v.type_ }

let find_value items name =
  List.find_opt
    (fun item ->
      match item with Value v when v.name = name -> true | _ -> false)
    items
  |> Option.map value
...
let cmd (items : Translated.structure_item list) : Ast3.cmd =
  ...
  let is_stmable (v : Translated.value) =
    v.name <> "Init_sut"
    && List.length v.arguments >= 1
    && List.(
         length
           (find_all
              (fun (arg : Translated.ocaml_var) -> arg.type_.name = "t")
              v.arguments))
       = 1
    && List.for_all
         (fun (ret : Translated.ocaml_var) -> ret.type_.name <> "t")
         v.returns
  in
  ...
  let make_stmable (v : Translated.value) =
    ...
    ( { index; arg = arg.name },
      { v with arguments = List.map snd (List.remove_assoc index args) } )
  in
  let out =
    List.fold_right
      (fun item acc ->
        ...
        | Value v when is_stmable v -> (
              ...
              safe_add v.name
                {
                  ...
                  ret =
                    (match v.returns with
                    | [] ->
                        [
                          {
                            name = Phase1.new_name "ret";
                            label = Nolabel;
                            typ = Unit;
                          };
                        ]
            ...
            | `Ok out -> out
            | `Duplicate key ->
                raise (Failure ("function declared twice: " ^ key)))
        | _ -> acc)
      items S.empty
  in
  if S.cardinal out = 0 then
    raise (Failure "no functions suitable for api found")
  else out
...
let state items : Ast3.state =
  match
    List.find_opt
      (fun s ->
        match s with Type t when String.equal t.name "t" -> true | _ -> false)
      items
  with
  | Some (Type t) ->
      List.fold_right
...
let get_sides (exp : expression) : (string * expression) option =
  let rec get_ident (exp : expression) : string option =
    let rec get_ident_li (li : longident) =
      match li with
      | Lident s -> s
      | Ldot (li, s) -> Printf.sprintf "%s.%s" (get_ident_li li) s
      | Lapply (li1, li2) ->
          Printf.sprintf "(%s, %s)" (get_ident_li li1) (get_ident_li li2)
    in
    match exp.pexp_desc with
    | Pexp_ident longident -> (
        match longident.txt with Lident s -> Some s | _ -> None)
    | Pexp_field (exp, li) ->
        Option.map
    ...
    | _ -> None
  in
  let is_equal (exp : expression) : bool =
    match exp.pexp_desc with
    | Pexp_ident longident -> (
        match longident.txt with Lident "=" -> true | _ -> false)
    | _ -> false
  in
  match exp.pexp_desc with
...
let make_state ?(init_state = false) (cmd_item : Translated.value)
    (state : state) (prefix : string) (used : bool I.t) :
    expression S.t * bool I.t =
  (*the bool for each equation is whether that equation can be used in make_next
  *)
  let get_field_rhs ?(error = "Unknown")
      (equations : Translated.postcondition list) (field : string)
    ...
    match
      List.find_opt
        ...
        (enum equations)
    with
    (*found a term which sets the field name equal to something*)
    | Some (i, postcond) ->
        Some (i, postcond.post.translation |> Result.get_ok |> get_rhs_exn)
    | None -> (
        match
          List.find_opt
            (fun postcond ->
        ...
        with
        | None ->
            Printf.eprintf "warning:field %s undefined in %s\n%!" field error;
            None
        | Some postcond ->
            Printf.eprintf
              "warning: field %s undefined in %s\n\
               (Postcondition %s cannot be used because it refers to the \
               return value,\n\
  ...
  S.fold
    (fun field _ (next_state, used) ->
      if cmd_item.pure then (next_state, used)
      else
        match
          get_field_rhs ~error:cmd_item.name cmd_item.postconditions field
            prefix
        with
...
let mk_used_posts posts =
  List.fold_right
    (fun i acc -> I.add i false acc)
    (List.init (List.length posts) (fun i -> i))
    I.empty

let init_state (items : Translated.structure_item list) (state : state) :
    init_state =
  match find_value items "Init_sut" with
  | Some cmd_item ->
      assert (List.length cmd_item.returns = 1);
      make_state ~init_state:true cmd_item state (List.hd cmd_item.returns).name
        (mk_used_posts cmd_item.postconditions)
      |> fst
  | None -> raise (Failure "init_sut undefined; could not initialize")

let translate_checks =
  List.map (fun check -> check.translations |> Result.get_ok |> fst)
  ...
  let zipped =
    S.mapi
      (fun cmd (cmd_ele : cmd_ele) ->
        let cmd_item =
          match find_value items cmd with
        ...
        let next, used_post =
          make_state cmd_item state cmd_ele.targ.arg used_post
        in
        ({ pres; next }, used_post))
      cmds
  in
...
let run items (cmds : cmd) : run =
  S.mapi
    (fun cmd (cmd_ele : cmd_ele) ->
      let cmd_item = find_value items cmd |> Option.get in
      (cmd_ele.ret, cmd_item.pure))
    cmds

let precond items cmds : precond =
  S.mapi
    (fun cmd _ ->
      let cmd_item = find_value items cmd |> Option.get in
      List.map
        (fun (term : Translated.term) -> term.translation |> Result.get_ok)
        cmd_item.preconditions)
    cmds
          ...
          (fun xpost ->
            {
              name = xpost.exn;
              args = xpost.args;
              translation = Result.get_ok xpost.translation;
            })
          cmd_item.xpostconditions
      in
      let ensures =
        let used = S.find cmd used in
        List.filter_map
          (fun (i, postcond) ->
