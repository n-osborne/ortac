  ...
  struct
        ...
        make ~print:show_cmd
          (let open Gen in
             oneof_weighted
               ...
               (1, ((with_flag Seq) <$> (pure Pop_all)));
               (1, ((with_flag Seq) <$> ((pure (fun x -> Push x)) <*> int)));
               (1,
                 ((with_flag Seq) <$>
    ...
    let next_state cmd__002_ state__003_ =
      ...
      | Of_list xs ->
          ...
          if cmd__002_.flag = Seq
          then Model.push (Model.drop_n state__003_ 0) a_2__007_
          else Model.drop_n state__003_ 0
      | Is_empty ->
    ...
    let run cmd__048_ sut__049_ =
      match cmd__048_.raw_cmd with
      | Create () ->
          Res
            (sut,
              (let res__050_ = create () in
               (if cmd__048_.flag = Seq
                then SUT.push sut__049_ res__050_
                else ();
                res__050_)))
      | Of_list xs ->
          Res
            (sut,
              (let res__051_ = of_list xs in
               (if cmd__048_.flag = Seq
                then SUT.push sut__049_ res__051_
                else ();
                res__051_)))
      | Is_empty ->
          Res
...
let ortac_show_cmd cmd__065_ models__066_ last__068_ res__067_ =
  let open Spec in
    let open STM in
      match ((cmd__065_.raw_cmd), res__067_) with
