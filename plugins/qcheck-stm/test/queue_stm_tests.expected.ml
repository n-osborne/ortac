...
module Spec =
  struct
      ...
      let open QCheck in
        let open Gen in
          oneof_weighted
            ...
            (1, (pure Length));
            (1, (pure Transfer))]
    let gen_cmd state__149_ =
      let open QCheck.Gen in (with_flag Seq) <$> (gen_cmd state__149_)
      ...
      | Copy ->
          ...
          then
            Model.push (Model.push (Model.drop_n state__007_ 1) t_11__032_)
              r__033_
          else Model.push (Model.drop_n state__007_ 1) t_11__032_
      | Is_empty ->
    ...
    let run cmd__114_ sut__115_ =
      match cmd__114_.raw_cmd with
      | Create () ->
          Res
            (sut,
              (let res__116_ = create () in
               (if cmd__114_.flag = Seq
                then SUT.push sut__115_ res__116_
                else ();
                res__116_)))
      | Add v ->
          Res
          ...
          Res
            (sut,
              (let t_11__135_ = SUT.get sut__115_ 0 in
               ...
               (if cmd__114_.flag = Seq
                then SUT.push sut__115_ res__136_
                else ();
                res__136_)))
      | Is_empty ->
          Res
...
let ortac_show_cmd cmd__145_ models__146_ last__148_ res__147_ =
  let open Spec in
    let open STM in
      match ((cmd__145_.raw_cmd), res__147_) with
...
let ortac_postcond cmd__042_ state__043_ res__044_ =
  let open Spec in
    let open STM in
      ...
      match ((cmd__042_.raw_cmd), res__044_) with
