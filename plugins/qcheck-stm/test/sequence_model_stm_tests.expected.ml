...
module Spec =
  struct
        ...
        make ~print:show_cmd
          (let open Gen in
             oneof_weighted
               ...
               (1, ((with_flag Seq) <$> ((pure (fun v -> Add v)) <*> char)));
               (1, ((with_flag Seq) <$> (pure Remove)));
               (1, ((with_flag Seq) <$> (pure Remove_)))])
    ...
    let run cmd__023_ sut__024_ =
      match cmd__023_.raw_cmd with
      | Create () ->
          Res
            (sut,
              (let res__025_ = create () in
               (if cmd__023_.flag = Seq
                then SUT.push sut__024_ res__025_
                else ();
                res__025_)))
      | Add v ->
          Res
