(* This file is generated by ortac qcheck-stm,
   edit how you run the tool instead *)
[@@@ocaml.warning "-26-27-69-32-34-37-38"]
open Tuples
module Ortac_runtime = Ortac_runtime_qcheck_stm_sequential
module SUT =
  (Ortac_runtime.SUT.Make)(struct
                             type sut = (char, int) t
                             let init () = create ()
                           end)
module ModelElt =
  struct
    type nonrec elt =
      {
      contents: (char * int) Ortac_runtime.Gospelstdlib.sequence }
    let init =
      let () = () in
      {
        contents =
          (try Ortac_runtime.Gospelstdlib.Sequence.empty
           with
           | e ->
               raise
                 (Ortac_runtime.Partial_function
                    (e,
                      {
                        Ortac_runtime.start =
                          {
                            pos_fname = "./tuples.mli";
                            pos_lnum = 6;
                            pos_bol = 267;
                            pos_cnum = 292
                          };
                        Ortac_runtime.stop =
                          {
                            pos_fname = "./tuples.mli";
                            pos_lnum = 6;
                            pos_bol = 267;
                            pos_cnum = 306
                          }
                      })))
      }
  end
module Model = (Ortac_runtime.Model.Make)(ModelElt)
module Spec =
  struct
    open STM
    type _ ty +=  
      | Integer: Ortac_runtime.integer ty 
    let integer = (Integer, Ortac_runtime.string_of_integer)
    type _ ty +=  
      | SUT: SUT.elt ty 
    let sut = (SUT, (fun _ -> "<sut>"))
    type _ ty +=  
      | Tup2: 'a1 ty * 'a2 ty -> ('a1 * 'a2) ty 
      | Tup3: 'a1 ty * 'a2 ty * 'a3 ty -> ('a1 * 'a2 * 'a3) ty 
    let tup2 spec1 spec2 =
      let (ty1, show1) = spec1
      and (ty2, show2) = spec2 in
      ((Tup2 (ty1, ty2)),
        (Util.Pp.to_show
           (Util.Pp.pp_tuple2 (Util.Pp.of_show show1) (Util.Pp.of_show show2))))
    and tup3 spec1 spec2 spec3 =
      let (ty1, show1) = spec1
      and (ty2, show2) = spec2
      and (ty3, show3) = spec3 in
      ((Tup3 (ty1, ty2, ty3)),
        (Util.Pp.to_show
           (Util.Pp.pp_tuple3 (Util.Pp.of_show show1) (Util.Pp.of_show show2)
              (Util.Pp.of_show show3))))
    type sut = SUT.t
    let init_sut = SUT.create 1
    type state = Model.t
    let init_state = Model.create 1 ()
    type raw_cmd =
      | Create of unit 
      | Clear 
      | Add of (char * int) 
      | Add' of (bool * char * int) 
      | Add'' of (bool * (char * int)) 
      | Size_tup 
      | Size_tup' 
    type flag =
      | Seq 
      | Dom 
    type cmd = {
      flag: flag ;
      raw_cmd: raw_cmd }
    let with_flag flag raw_cmd = { flag; raw_cmd }
    let show_cmd cmd__005_ =
      match cmd__005_.raw_cmd with
      | Create () ->
          Format.asprintf "%s %a" "create" (Util.Pp.pp_unit true) ()
      | Clear -> Format.asprintf "%s <sut>" "clear"
      | Add tup ->
          Format.asprintf "%s <sut> %a" "add"
            (Util.Pp.pp_tuple2 Util.Pp.pp_char Util.Pp.pp_int true) tup
      | Add' tup_1 ->
          Format.asprintf "%s <sut> %a" "add'"
            (Util.Pp.pp_tuple3 Util.Pp.pp_bool Util.Pp.pp_char Util.Pp.pp_int
               true) tup_1
      | Add'' tup_2 ->
          Format.asprintf "%s <sut> %a" "add''"
            (Util.Pp.pp_tuple2 Util.Pp.pp_bool
               (Util.Pp.pp_tuple2 Util.Pp.pp_char Util.Pp.pp_int) true) tup_2
      | Size_tup -> Format.asprintf "%s <sut>" "size_tup"
      | Size_tup' -> Format.asprintf "%s <sut>" "size_tup'"
    let cleanup _ = ()
    let gen_cmd _ =
      let open QCheck in
        let open Gen in
          oneof_weighted
            [(1, ((pure (fun () -> Create ())) <*> unit));
            (1, (pure Clear));
            (1, ((pure (fun tup -> Add tup)) <*> (tup2 char int)));
            (1, ((pure (fun tup_1 -> Add' tup_1)) <*> (tup3 bool char int)));
            (1,
              ((pure (fun tup_2 -> Add'' tup_2)) <*>
                 (tup2 bool (tup2 char int))));
            (1, (pure Size_tup));
            (1, (pure Size_tup'))]
    let gen_cmd state__064_ =
      let open QCheck.Gen in (with_flag Seq) <$> (gen_cmd state__064_)
    let arb_cmd state__001_ =
      let open QCheck in make ~print:show_cmd (gen_cmd state__001_)
    let next_state cmd__006_ state__007_ =
      match cmd__006_.raw_cmd with
      | Create () ->
          let h__009_ =
            let open ModelElt in
              {
                contents =
                  (try Ortac_runtime.Gospelstdlib.Sequence.empty
                   with
                   | e ->
                       raise
                         (Ortac_runtime.Partial_function
                            (e,
                              {
                                Ortac_runtime.start =
                                  {
                                    pos_fname = "./tuples.mli";
                                    pos_lnum = 6;
                                    pos_bol = 267;
                                    pos_cnum = 292
                                  };
                                Ortac_runtime.stop =
                                  {
                                    pos_fname = "./tuples.mli";
                                    pos_lnum = 6;
                                    pos_bol = 267;
                                    pos_cnum = 306
                                  }
                              })))
              } in
          if cmd__006_.flag = Seq
          then Model.push (Model.drop_n state__007_ 0) h__009_
          else Model.drop_n state__007_ 0
      | Clear ->
          let h_1__010_ = Model.get state__007_ 0 in
          let h_1__011_ =
            let open ModelElt in
              {
                contents =
                  (try Ortac_runtime.Gospelstdlib.Sequence.empty
                   with
                   | e ->
                       raise
                         (Ortac_runtime.Partial_function
                            (e,
                              {
                                Ortac_runtime.start =
                                  {
                                    pos_fname = "./tuples.mli";
                                    pos_lnum = 11;
                                    pos_bol = 460;
                                    pos_cnum = 485
                                  };
                                Ortac_runtime.stop =
                                  {
                                    pos_fname = "./tuples.mli";
                                    pos_lnum = 11;
                                    pos_bol = 460;
                                    pos_cnum = 499
                                  }
                              })))
              } in
          Model.push (Model.drop_n state__007_ 1) h_1__011_
      | Add tup ->
          let h_2__012_ = Model.get state__007_ 0 in
          let h_2__013_ =
            let open ModelElt in
              {
                contents =
                  (try
                     match tup with
                     | (a_1, b_1) ->
                         Ortac_runtime.Gospelstdlib.Sequence.cons (a_1, b_1)
                           h_2__012_.contents
                   with
                   | e ->
                       raise
                         (Ortac_runtime.Partial_function
                            (e,
                              {
                                Ortac_runtime.start =
                                  {
                                    pos_fname = "./tuples.mli";
                                    pos_lnum = 16;
                                    pos_bol = 666;
                                    pos_cnum = 691
                                  };
                                Ortac_runtime.stop =
                                  {
                                    pos_fname = "./tuples.mli";
                                    pos_lnum = 16;
                                    pos_bol = 666;
                                    pos_cnum = 751
                                  }
                              })))
              } in
          Model.push (Model.drop_n state__007_ 1) h_2__013_
      | Add' tup_1 ->
          let h_3__014_ = Model.get state__007_ 0 in
          let h_3__015_ =
            let open ModelElt in
              {
                contents =
                  (try
                     match tup_1 with
                     | (c, a_2, b_2) ->
                         if c = true
                         then
                           Ortac_runtime.Gospelstdlib.Sequence.cons
                             (a_2, b_2) h_3__014_.contents
                         else h_3__014_.contents
                   with
                   | e ->
                       raise
                         (Ortac_runtime.Partial_function
                            (e,
                              {
                                Ortac_runtime.start =
                                  {
                                    pos_fname = "./tuples.mli";
                                    pos_lnum = 21;
                                    pos_bol = 973;
                                    pos_cnum = 998
                                  };
                                Ortac_runtime.stop =
                                  {
                                    pos_fname = "./tuples.mli";
                                    pos_lnum = 23;
                                    pos_bol = 1080;
                                    pos_cnum = 1107
                                  }
                              })))
              } in
          Model.push (Model.drop_n state__007_ 1) h_3__015_
      | Add'' tup_2 ->
          let h_4__016_ = Model.get state__007_ 0 in
          let h_4__017_ =
            let open ModelElt in
              {
                contents =
                  (try
                     match tup_2 with
                     | (c_1, (a_3, b_3)) ->
                         if c_1 = true
                         then
                           Ortac_runtime.Gospelstdlib.Sequence.cons
                             (a_3, b_3) h_4__016_.contents
                         else h_4__016_.contents
                   with
                   | e ->
                       raise
                         (Ortac_runtime.Partial_function
                            (e,
                              {
                                Ortac_runtime.start =
                                  {
                                    pos_fname = "./tuples.mli";
                                    pos_lnum = 28;
                                    pos_bol = 1275;
                                    pos_cnum = 1300
                                  };
                                Ortac_runtime.stop =
                                  {
                                    pos_fname = "./tuples.mli";
                                    pos_lnum = 30;
                                    pos_bol = 1384;
                                    pos_cnum = 1411
                                  }
                              })))
              } in
          Model.push (Model.drop_n state__007_ 1) h_4__017_
      | Size_tup ->
          let t_1__018_ = Model.get state__007_ 0 in
          let t_1__019_ = t_1__018_ in
          Model.push (Model.drop_n state__007_ 1) t_1__019_
      | Size_tup' ->
          let t_2__020_ = Model.get state__007_ 0 in
          let t_2__021_ = t_2__020_ in
          Model.push (Model.drop_n state__007_ 1) t_2__021_
    let precond cmd__042_ state__043_ =
      match cmd__042_.raw_cmd with
      | Create () -> true
      | Clear -> true
      | Add tup -> true
      | Add' tup_1 -> true
      | Add'' tup_2 -> true
      | Size_tup -> true
      | Size_tup' -> true
    let postcond _ _ _ = true
    let run cmd__044_ sut__045_ =
      match cmd__044_.raw_cmd with
      | Create () ->
          Res
            (sut,
              (let res__046_ = create () in
               (if cmd__044_.flag = Seq
                then SUT.push sut__045_ res__046_
                else ();
                res__046_)))
      | Clear ->
          Res
            (unit,
              (let h_1__047_ = SUT.get sut__045_ 0 in
               let res__048_ = clear h_1__047_ in res__048_))
      | Add tup ->
          Res
            (unit,
              (let h_2__049_ = SUT.get sut__045_ 0 in
               let res__050_ = add h_2__049_ tup in res__050_))
      | Add' tup_1 ->
          Res
            (unit,
              (let h_3__051_ = SUT.get sut__045_ 0 in
               let res__052_ = add' h_3__051_ tup_1 in res__052_))
      | Add'' tup_2 ->
          Res
            (unit,
              (let h_4__053_ = SUT.get sut__045_ 0 in
               let res__054_ = add'' h_4__053_ tup_2 in res__054_))
      | Size_tup ->
          Res
            ((tup2 int int),
              (let t_1__055_ = SUT.get sut__045_ 0 in
               let res__056_ = size_tup t_1__055_ in res__056_))
      | Size_tup' ->
          Res
            ((tup3 int int int),
              (let t_2__057_ = SUT.get sut__045_ 0 in
               let res__058_ = size_tup' t_2__057_ in res__058_))
  end
module STMTests = (Ortac_runtime.Make)(Spec)
let check_init_state () = ()
let ortac_show_cmd cmd__060_ models__061_ last__063_ res__062_ =
  let open Spec in
    let open STM in
      match ((cmd__060_.raw_cmd), res__062_) with
      | (Create (), Res ((SUT, _), h)) ->
          let lhs = if last__063_ then "r" else Model.get_name models__061_ 0
          and shift = 1 in
          Format.asprintf "let %s = %s %a" lhs "create"
            (Util.Pp.pp_unit true) ()
      | (Clear, Res ((Unit, _), _)) ->
          let lhs = if last__063_ then "r" else "_"
          and shift = 0 in
          Format.asprintf "let %s = %s %s" lhs "clear"
            (Model.get_name models__061_ (0 + shift))
      | (Add tup, Res ((Unit, _), _)) ->
          let lhs = if last__063_ then "r" else "_"
          and shift = 0 in
          Format.asprintf "let %s = %s %s %a" lhs "add"
            (Model.get_name models__061_ (0 + shift))
            (Util.Pp.pp_tuple2 Util.Pp.pp_char Util.Pp.pp_int true) tup
      | (Add' tup_1, Res ((Unit, _), _)) ->
          let lhs = if last__063_ then "r" else "_"
          and shift = 0 in
          Format.asprintf "let %s = %s %s %a" lhs "add'"
            (Model.get_name models__061_ (0 + shift))
            (Util.Pp.pp_tuple3 Util.Pp.pp_bool Util.Pp.pp_char Util.Pp.pp_int
               true) tup_1
      | (Add'' tup_2, Res ((Unit, _), _)) ->
          let lhs = if last__063_ then "r" else "_"
          and shift = 0 in
          Format.asprintf "let %s = %s %s %a" lhs "add''"
            (Model.get_name models__061_ (0 + shift))
            (Util.Pp.pp_tuple2 Util.Pp.pp_bool
               (Util.Pp.pp_tuple2 Util.Pp.pp_char Util.Pp.pp_int) true) tup_2
      | (Size_tup, Res ((Tup2 (Int, Int), _), _)) ->
          let lhs = if last__063_ then "r" else "_"
          and shift = 0 in
          Format.asprintf "let %s = %s %s" lhs "size_tup"
            (Model.get_name models__061_ (0 + shift))
      | (Size_tup', Res ((Tup3 (Int, Int, Int), _), _)) ->
          let lhs = if last__063_ then "r" else "_"
          and shift = 0 in
          Format.asprintf "let %s = %s %s" lhs "size_tup'"
            (Model.get_name models__061_ (0 + shift))
      | _ -> assert false
let ortac_postcond cmd__022_ state__023_ res__024_ =
  let open Spec in
    let open STM in
      let new_state__025_ = lazy (next_state cmd__022_ state__023_) in
      match ((cmd__022_.raw_cmd), res__024_) with
      | (Create (), Res ((SUT, _), h)) -> None
      | (Clear, Res ((Unit, _), _)) -> None
      | (Add tup, Res ((Unit, _), _)) -> None
      | (Add' tup_1, Res ((Unit, _), _)) -> None
      | (Add'' tup_2, Res ((Unit, _), _)) -> None
      | (Size_tup, Res ((Tup2 (Int, Int), _), (x, y))) ->
          Ortac_runtime.Report.append
            (if
               let t_old__030_ = Model.get state__023_ 0
               and t_new__031_ =
                 lazy (Model.get (Lazy.force new_state__025_) 0) in
               try
                 (Ortac_runtime.Gospelstdlib.integer_of_int x) =
                   (Ortac_runtime.Gospelstdlib.Sequence.length
                      (Lazy.force t_new__031_).contents)
               with | e -> false
             then None
             else
               Some
                 (Ortac_runtime.Report.report "Tuples" "create ()"
                    (try
                       Ortac_runtime.Report.Value
                         (Res (Ortac_runtime.Report.dummy, ()))
                     with | e -> Ortac_runtime.Report.Out_of_domain)
                    "size_tup"
                    [("x = Sequence.length t.contents",
                       {
                         Ortac_runtime.start =
                           {
                             pos_fname = "./tuples.mli";
                             pos_lnum = 34;
                             pos_bol = 1558;
                             pos_cnum = 1570
                           };
                         Ortac_runtime.stop =
                           {
                             pos_fname = "./tuples.mli";
                             pos_lnum = 34;
                             pos_bol = 1558;
                             pos_cnum = 1600
                           }
                       })]))
            (if
               let t_old__032_ = Model.get state__023_ 0
               and t_new__033_ =
                 lazy (Model.get (Lazy.force new_state__025_) 0) in
               try
                 (Ortac_runtime.Gospelstdlib.integer_of_int y) =
                   (Ortac_runtime.Gospelstdlib.Sequence.length
                      (Lazy.force t_new__033_).contents)
               with | e -> false
             then None
             else
               Some
                 (Ortac_runtime.Report.report "Tuples" "create ()"
                    (try
                       Ortac_runtime.Report.Value
                         (Res (Ortac_runtime.Report.dummy, ()))
                     with | e -> Ortac_runtime.Report.Out_of_domain)
                    "size_tup"
                    [("y = Sequence.length t.contents",
                       {
                         Ortac_runtime.start =
                           {
                             pos_fname = "./tuples.mli";
                             pos_lnum = 35;
                             pos_bol = 1601;
                             pos_cnum = 1613
                           };
                         Ortac_runtime.stop =
                           {
                             pos_fname = "./tuples.mli";
                             pos_lnum = 35;
                             pos_bol = 1601;
                             pos_cnum = 1643
                           }
                       })]))
      | (Size_tup', Res ((Tup3 (Int, Int, Int), _), (x_1, y_1, z))) ->
          Ortac_runtime.Report.append
            (if
               let t_old__035_ = Model.get state__023_ 0
               and t_new__036_ =
                 lazy (Model.get (Lazy.force new_state__025_) 0) in
               try
                 (Ortac_runtime.Gospelstdlib.integer_of_int x_1) =
                   (Ortac_runtime.Gospelstdlib.Sequence.length
                      (Lazy.force t_new__036_).contents)
               with | e -> false
             then None
             else
               Some
                 (Ortac_runtime.Report.report "Tuples" "create ()"
                    (try
                       Ortac_runtime.Report.Value
                         (Res (Ortac_runtime.Report.dummy, ()))
                     with | e -> Ortac_runtime.Report.Out_of_domain)
                    "size_tup'"
                    [("x = Sequence.length t.contents",
                       {
                         Ortac_runtime.start =
                           {
                             pos_fname = "./tuples.mli";
                             pos_lnum = 39;
                             pos_bol = 1816;
                             pos_cnum = 1828
                           };
                         Ortac_runtime.stop =
                           {
                             pos_fname = "./tuples.mli";
                             pos_lnum = 39;
                             pos_bol = 1816;
                             pos_cnum = 1858
                           }
                       })]))
            (Ortac_runtime.Report.append
               (if
                  let t_old__037_ = Model.get state__023_ 0
                  and t_new__038_ =
                    lazy (Model.get (Lazy.force new_state__025_) 0) in
                  try
                    (Ortac_runtime.Gospelstdlib.integer_of_int y_1) =
                      (Ortac_runtime.Gospelstdlib.Sequence.length
                         (Lazy.force t_new__038_).contents)
                  with | e -> false
                then None
                else
                  Some
                    (Ortac_runtime.Report.report "Tuples" "create ()"
                       (try
                          Ortac_runtime.Report.Value
                            (Res (Ortac_runtime.Report.dummy, ()))
                        with | e -> Ortac_runtime.Report.Out_of_domain)
                       "size_tup'"
                       [("y = Sequence.length t.contents",
                          {
                            Ortac_runtime.start =
                              {
                                pos_fname = "./tuples.mli";
                                pos_lnum = 40;
                                pos_bol = 1859;
                                pos_cnum = 1871
                              };
                            Ortac_runtime.stop =
                              {
                                pos_fname = "./tuples.mli";
                                pos_lnum = 40;
                                pos_bol = 1859;
                                pos_cnum = 1901
                              }
                          })]))
               (if
                  let t_old__039_ = Model.get state__023_ 0
                  and t_new__040_ =
                    lazy (Model.get (Lazy.force new_state__025_) 0) in
                  try
                    (Ortac_runtime.Gospelstdlib.integer_of_int z) =
                      (Ortac_runtime.Gospelstdlib.Sequence.length
                         (Lazy.force t_new__040_).contents)
                  with | e -> false
                then None
                else
                  Some
                    (Ortac_runtime.Report.report "Tuples" "create ()"
                       (try
                          Ortac_runtime.Report.Value
                            (Res (Ortac_runtime.Report.dummy, ()))
                        with | e -> Ortac_runtime.Report.Out_of_domain)
                       "size_tup'"
                       [("z = Sequence.length t.contents",
                          {
                            Ortac_runtime.start =
                              {
                                pos_fname = "./tuples.mli";
                                pos_lnum = 41;
                                pos_bol = 1902;
                                pos_cnum = 1914
                              };
                            Ortac_runtime.stop =
                              {
                                pos_fname = "./tuples.mli";
                                pos_lnum = 41;
                                pos_bol = 1902;
                                pos_cnum = 1944
                              }
                          })])))
      | _ -> None
let _ =
  QCheck_base_runner.run_tests_main
    (let count = 1000 in
     [STMTests.agree_test ~count ~name:"Tuples STM tests" 1 check_init_state
        ortac_show_cmd ortac_postcond])
