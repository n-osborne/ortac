...
val length : 'a t -> int
(** Returns the number of elemenets in the given sequence. This is a O(n)
...
(** [take_opt_l x s] remove and returns [Some x] where [x] is the rightmost
    element of [s] or [None] if [s] is empty *)
(*@ o = take_opt_r s
    modifies s.contents
    ensures s.contents = if Sequence.length (old s.contents) = 0
...
(** [transfer_l s1 s2] removes all elements of [s1] and add them at the left of
    [s2]. This operation runs in constant time and space. *)

val transfer_r : 'a t -> 'a t -> unit
(** [transfer_r s1 s2] removes all elements of [s1] and add them at the right of
    [s2]. This operation runs in constant time and space. *)
...
(** [find_node_opt_l f s] returns [Some x], where [x] is the first node of [s]
    starting from the left that satisfies [f] or [None] if none exists. *)

val find_node_opt_r : ('a -> bool) -> 'a t -> 'a node option
(** [find_node_opt_r f s] returns [Some x], where [x] is the first node of [s]
    starting from the right that satisfies [f] or [None] if none exists. *)

val find_node_l : ('a -> bool) -> 'a t -> 'a node
(** [find_node_l f s] returns the first node of [s] starting from the left that
    satisfies [f] or raises [Not_found] if none exists. *)

val find_node_r : ('a -> bool) -> 'a t -> 'a node
(** [find_node_r f s] returns the first node of [s] starting from the right that
    satisfies [f] or raises [Not_found] if none exists. *)
